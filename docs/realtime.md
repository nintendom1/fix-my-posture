...
- Motion sampling and its 30 Hz display timer follow the capture lifecycle. They stop on Details, dismissal, backgrounding, authorization/setup failure, runtime failure, and interruption, then restart only when an active interrupted session resumes. Display polling never changes camera direction or invalidates a sensor sample.
- Camera tilt compensation defaults on and is available from both the home camera-settings menu and realtime controls. The horizon line has an independent persistent setting and is clipped to the preview's aspect-fit image rectangle. If motion is unavailable, live analysis falls back to image axes and displays a warning.
